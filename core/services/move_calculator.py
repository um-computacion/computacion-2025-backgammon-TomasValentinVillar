...
class MoveCalculator:
    """
    Responsabilidad: Calcular movimientos posibles
    ...
    def _hay_movimientos_desde_inicio(self, board, turno, dice_manager):
        ...
        if dado1_val != dado2_val:
            ...
            pos_destino = self.__move_validator__.calcular_destino(pos_inicio, pasos_combinados, turno)
    ...
    def _es_movimiento_valido(self, board, pos_origen, pasos, turno, dice_manager):
        ...
        if not self.__move_validator__.es_posicion_valida(pos_destino):
            try:
                ...
                dado1_temp = Dice()
                dado2_temp = Dice()
                dado1_temp.__numero__ = valores[0]
                dado2_temp.__numero__ = valores[1]
                
                self.__rule_validator__.puede_sacar_ficha(board, pos_origen, turno, dado1_temp, dado2_temp)
                return True
            except ValueError:
                return False
