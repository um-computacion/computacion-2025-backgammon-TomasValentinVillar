...
classDiagram
    %% DIAGRAMA SIMPLIFICADO CON ATRIBUTOS
    ...
    class BackgammonGame {
        ...
        -DiceManager dice_manager
        -MoveCalculator move_calculator
        -dict players
        +crear_jugador()
        +realizar_movimiento()
        +tirar_dados()
    ...
    class Board {
        -list contenedor_fichas
        -list contenedor_fichas_blancas_sacadas
        -list contenedor_fichas_negras_sacadas
        -list contenedor_fichas_blancas
        -list contenedor_fichas_negras
        +poner_ficha()
        +quitar_ficha()
        +obtener_contenedor_fichas()
    }

    class DiceManager {
        -Dice dice1
        -Dice dice2
        -list dados_disponibles
        +tirar_dados()
        +obtener_dados_disponibles()
        +usar_dado()
    }

    class MoveValidator {
        +es_posicion_disponible()
        +calcular_destino()
    }

    class RuleValidator {
        +puede_sacar_ficha()
        +tiene_fichas_comidas()
    }

    class MoveCalculator {
        -MoveValidator move_validator
        -RuleValidator rule_validator
        +hay_movimientos_posibles()
        +calcular_pasos_movimiento()
    }
    ...
    class BoardAdapter {
        -BackgammonGame backgammon_game
        -dict pos
        +actualizar()
    }
    ...
    BackgammonGame *-- DiceManager
    BackgammonGame *-- MoveValidator
    BackgammonGame *-- RuleValidator
    BackgammonGame *-- MoveCalculator
    BackgammonGame o-- Player

    Board o-- Checker

    DiceManager *-- Dice

    MoveCalculator --> MoveValidator
    MoveCalculator --> RuleValidator

    CLI --> BackgammonGame
    PygameUI --> BackgammonGame
    PygameUI --> BoardAdapter
    BoardAdapter --> BackgammonGame